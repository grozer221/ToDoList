...
import {
    ...
    TodosGetByIdData,
    TodosGetByIdVars
} from "../../gql/modules/todos/todos.queries";
import {
    ...
    TodosUpdateData,
    TodosUpdateVars
} from "../../gql/modules/todos/todos.mutations";
    ...
    action$.pipe(
        ...
        mergeMap(action =>
            from(client.mutate<TodosCreateData, TodosCreateVars>({
                ...
                variables: {toDosCreateInputType: action.payload}
            })).pipe(
    ...
    action$.pipe(
        ...
        mergeMap(action =>
            from(client.mutate<TodosUpdateData, TodosUpdateVars>({
                ...
                variables: {toDosUpdateInputType: action.payload}
            })).pipe(
                map(response => todosActions.setUpdatedTodo(response.data?.toDos.update as Todo)),
                catchError(error => of(todosActions.setFetchTodosError(error))),
